...
description: "`orc-cli-workflow`의 check 단계에서 호출되며, 새 tmux pane에서 `check-code`와 `orc-cli-workflow`를 사용해 코드 체크를 수행하고 `.project/feedback.md`를 갱신하며, feedback을 바탕으로 새 `plan.md`를 만든 뒤 남은 문제를 해결하는 followup agent다."
---
...
- followup agent는 feedback을 읽은 뒤 새 `plan.md`를 작성하고 그 계획으로 문제를 해결해야 한다.
...
4. followup prompt를 만든다.
   ...
   - feedback을 읽고 새 `plan.md`를 만든 뒤 문제를 해결하라는 직접 명령
...
6. 메인 pane은 아래 파일만 기준으로 진행 상태를 확인한다.
   - `.project/feedback.md`
   - `.project/check-process.md`
   - 검증 산출물 (`report.md`, screenshot, test output files)
...
- 먼저 `.project/feedback.md`를 읽고 새 `plan.md`를 만든다.
...
## pane 명령 예시
```text
cd <workdir> && codex exec --dangerously-bypass-approvals-and-sandbox "`.project/feedback.md`를 읽고 새로운 plan.md를 만든 뒤 문제를 해결하라. `check-code` skill과 `orc-cli-workflow` skill을 반드시 사용하라."
```

## 종료 게이트
- `.project/feedback.md`의 `# 문제` bullet 수가 0
- 새 `plan.md`가 followup 작업 기준으로 갱신됨
- 필요한 검증 명령 성공
