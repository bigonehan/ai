...
## Execution-Path Validation (Mandatory)
- 검증은 함수 단위가 아니라 실행 경로 단위로 확인한다.
- 최소 검증 경로를 반드시 점검한다.
  - 트리거 입력(키/명령)
  - 실제 호출된 액션/CLI
  - 파일/상태 전이(before -> after)
  - 후속 동작 결과(다음 단계가 정상 실행되는지)
- UI 작업은 가능하면 동등한 CLI 경로로 교차 검증한다.
- 상태바/모달 문구 변경만으로는 통과로 판단하지 않는다.
- 경로 중 어느 단계가 실패했는지 단계명을 명시해 보고한다.
...
## Draft Follow-up Contract
- `create-draft`/`add-function` 후속 점검 프롬프트의 세부 규칙은 코드에 하드코딩하지 않고 이 스킬을 참조한다.
- 대상 draft YAML을 점검할 때는 아래 실행 경로를 기준으로 검증한다.
  - 트리거(create-draft/add-function)
  - draft 파일 생성/수정 결과
  - 병렬 빌드 가능 여부
- 점검 시 YAML/Markdown 참조 파일이 있으면 먼저 읽고, 채워야 할 헤더/속성을 확인한 뒤 형식에 맞게 반영한다.
- 문제를 찾으면 파일을 직접 수정하고 변경 요약만 출력한다.
- 문제가 없으면 `NO_CHANGE`를 출력한다.
