...
- `.project/drafts.yaml` : 현재 기능 상태, 규칙, 의존관계 참조용
- `.project/feedback.md` : 남은 문제와 이미 해결된 문제 참조용
...
## 참조 순서
1. `.project/drafts.yaml`에서 프로젝트 규칙, 기능 상태, 의존관계를 확인한다
2. `.project/feedback.md`에서 남은 문제와 제약을 확인한다
3. `draft` 관련 구조는 `./.project/drafts.yaml` 형식을 기준으로 해석한다
...
- 실행 실패 또는 대기시간 초과 시 `./.project/feedback.md`에 문제를 기록한 뒤 실패 처리
- 전체 완료 후 구현 완료 항목을 `./.project/drafts.yaml` 상태(`planned/worked/complete/failed`)에 반영한다
- 해결된 문제는 `./.project/feedback.md`의 `# 해결`로 이동한다
...
# 완료 보고
- 완료된 draft는 `설치폴더/.project/drafts.yaml` 상태와 `설치폴더/.project/feedback.md`에 반영한다.
