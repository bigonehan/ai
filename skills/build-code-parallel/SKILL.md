...
3. `draft` 관련 구조는 `./.project/drafts.yaml`, `./.project/drafts_list.yaml` 형식을 기준으로 해석한다
...
- `./.project/drafts_list.yaml`에서 같은 기능을 `planned`에서 제거하고 `feature`에 추가한다
...
# 완료 보고
- 완료된 draft는 정리되고 `설치폴더/.project/drafts_list.yaml`의 `feature` 항목에 반영한다.
